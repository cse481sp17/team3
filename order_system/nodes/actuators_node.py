...
import order_system.msg
from order_system.srv import SendFetch, SendFetchResponse
...
class ActuatorServer(object):
	def __init__(self):
		...
		self.pose_pub = rospy.Publisher('/pose_names', order_system.msg.PoseNames, latch=True)
		...
		try:
			with open("/home/team3/catkin_ws/src/cse481c/order_service/LOL.pickle", "r") as f:
...
def main():
	rospy.init_node('order_system_actuators')
	wait_for_time()

	server = ActuatorServer()

	move_service = rospy.Service('order_system/send_fetch', SendFetch, server.handle_send_fetch)
